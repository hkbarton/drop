...
// pulse with manager node information, same as pulse, but pass with manager node information
...
function startPulseProcess(){
  subproc.pulseProcess = require('child_process').fork(
    path.join(__dirname, 'pulse.js'), [JSON.stringify(config)], {
    cwd:process.cwd()
  });
  subproc.pulseProcess.on('exit', pulseExitCallback);
  // handle message from pulse process
  subproc.pulseProcess.on('message', function(m){
    // re send message to worker
    subproc.workderProcess.send(m);
  });
}
...
function startWorkerProcess(){
  subproc.workderProcess = require('child_process').fork(
    path.join(__dirname, 'worker.js'), [JSON.stringify(config)], {
    cwd:process.cwd()
  });
  subproc.workderProcess.on('exit', workerExitCallback);
}
